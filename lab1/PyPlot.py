...
for line in open(data):
    if(line.strip()):
        data_input.append(list(map(float, list(
            filter(lambda x: x != '', (line.lstrip().rstrip()
                                       .split(sep=' ')))))))
...
plt.ylabel("y")
plt.show()
draw_plot(0, 100, 0, 100, a[:, 1:2], a[:, 2:3], a[:, 0:1],
          xlabel="X", ylabel="Y", title="First Quadrant Train")
draw_plot(0, 100, 0, 100, a[:, 1:2], a[:, 2:3], a[:, 0:1],
          xlabel="X", ylabel="Y", title="First Quadrant Answer")
draw_plot(0, -100, 0, 100, a[:, 1:2], a[:, 2:3], a[:, 0:1],
          xlabel="X", ylabel="Y", title="Second Quadrant Train")
draw_plot(0, -100, 0, 100, a[:, 1:2], a[:, 2:3], a[:, 0:1],
          xlabel="X", ylabel="Y", title="Second Quadrant Answer")
draw_plot(0, -100, 0, -100, a[:, 1:2], a[:, 2:3], a[:, 0:1],
          xlabel="X", ylabel="Y", title="Third Quadrant Train")
draw_plot(0, -100, 0, -100, a[:, 1:2], a[:, 2:3], a[:, 0:1],
          xlabel="X", ylabel="Y", title="Third Quadrant Answer")
draw_plot(0, 100, 0, -100, a[:, 1:2], a[:, 2:3], a[:, 0:1],
          xlabel="X", ylabel="Y", title="Fourth Quadrant Train")
draw_plot(0, 100, 0, -100, a[:, 1:2], a[:, 2:3], a[:, 0:1],
          xlabel="X", ylabel="Y", title="Fourth Quadrant Answer")
